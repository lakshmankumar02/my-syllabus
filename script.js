function selectCollegeName() {
  ...
  document.querySelector(".card_college_name").innerHTML = val;
}
...
function myFunction() {
  if (window.pageYOffset >= sticky) {
    ...
    homeSec.classList.add("pad-top");
  } else {
    navbar.classList.remove("sticky");
    homeSec.classList.remove("pad-top");
  }
}
...
document.querySelector(".ham").addEventListener("click", () => {
  document.querySelector("#menu_icon").classList.toggle("hamburger");
  if (document.querySelector("#menu_icon").classList.contains("hamburger")) {
...
icon = document.querySelector("#navlist_branch .icons");

burger.addEventListener("click", () => {
  navList.classList.toggle("v_class");
  navbar.classList.toggle("h_nav");
  icon.classList.toggle("v_class");
});

// for length of 4 for branch
let numberOfBranch = document.querySelectorAll(".branch_box").length;

// for length of 7 for syllabus
let numberOfSyllabus = document.querySelectorAll(".sbd_column").length;

// for length of 7 for semester list item
let numberOfSemesterListItem =
  document.querySelectorAll(".semester_list li").length / 4;

// for length of 4 for semester list
let numberOfSemesterList = document.querySelectorAll(".semester_list").length;

// for length of 7 for share button
let numberOfShare = document.querySelectorAll(".sbd_share").length;
...
let branchBox = ["civil", "computer", "ece", "mechanical"];

// scale ka khel

for (let i = 0; i < numberOfSemesterList; i++) {
  let docSelec = branchBox[i];
  let docSelecName = "#semester_list_" + docSelec + " li";
  for (let i = 0; i < numberOfSemesterListItem; i++) {
    let semTime = syllabusTime[i];
    let semNum = "#syllabus_" + semTime;
    document
      .querySelectorAll(docSelecName)
      [i].addEventListener("click", function () {
        let ssC = document.querySelector(semNum);
        ssC.classList.add("scale");
        setTimeout(() => {
          ssC.classList.remove("scale");
        }, 2000);
      });
  }
}
...
for (let i = 0; i < numberOfBranch; i++) {
  ...
  // yaha dot isliye diya gaya hai taki jab navigation bar wale branch par bhi click kiya jaye to ye work kare
  ...
  $(idBBN).click(function () {
    ...
    $(".card_branch_name").text(syllabusBranchName);
    ...
    for (let i = 0; i < numberOfSyllabus; i++) {
      ...
      let syllabusDownloads = document.querySelectorAll(".sbd_downloads a");
...
function validateForm() {
  ...
  if (name.length < 3) {
    seterror("fname", "*Minimum Length should be 3");
    returnval = false;
  }
  ...
  if (subject.length < 5) {
    seterror("fname", "*Minimum length should be 5");
    returnval = false;
  }
